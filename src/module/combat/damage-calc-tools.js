...
Hooks.on('renderChatMessage', (message, html, data) => {
    setTimeout(() => {
        $(html)
            .find('.apply-damage-button')
            .on('click', game.ptu.utils.combat.applyDamageToTargets)
        $(html)
            .find('.undo-damage-button')
            .on('click', game.ptu.utils.combat.undoDamageToTargets)
        $(html)
            .find('.half-damage-button')
            .on('click', (ev) =>
                game.ptu.utils.combat.applyDamageToTargets(
                    ev,
                    ATTACK_MOD_OPTIONS.HALF
                )
            )
        $(html)
            .find('.resist-damage-button')
            .on('click', (ev) =>
                game.ptu.utils.combat.applyDamageToTargets(
                    ev,
                    ATTACK_MOD_OPTIONS.RESIST
                )
            )
        $(html)
            .find('.flat-damage-button')
            .on('click', (ev) =>
                game.ptu.utils.combat.applyDamageToTargets(
                    ev,
                    ATTACK_MOD_OPTIONS.FLAT
                )
            )
        $(html)
            .find('.automated-damage-button')
            .click(applyDamageAndEffectsToTargets)
        $(html).find('.automated-effect-button').click(applyEffectsToTargets)
        $(html)
            .find('.mon-item')
            .click(game.ptu.utils.combat.handleApplicatorItem)
    }, 500)
})
...
export async function applyDamageToTargets(
    event,
    options = ATTACK_MOD_OPTIONS.NONE
) {
    event.preventDefault()
    if (event.target != event.currentTarget) return

    const dataset = event.currentTarget.dataset.moveName
    ...
    if (event.shiftKey) {
        dr = await new Promise((resolve, reject) => {
            Dialog.confirm({
                ...
                yes: async (html) => {
                    const bonusTxt = html
                        .find('input[name="damage-reduction"]')
                        .val()

                    const bonus = !isNaN(Number(bonusTxt))
                        ? Number(bonusTxt)
                        : parseInt(
                              (await new Roll(bonusTxt).roll({ async: true }))
                                  .total
                          )
                    if (!isNaN(bonus)) {
    ...
    if (options > 0) {
        switch (options) {
            case ATTACK_MOD_OPTIONS.HALF:
                return executeApplyDamageToTargets(
                    canvas.tokens.controlled,
                    moveData,
                    !moveData.isCrit
                        ? Math.max(1, Math.floor(moveData.regDamage / 2))
                        : Math.max(1, Math.floor(moveData.critDamage / 2)),
                    { damageReduction: dr }
                )
            case ATTACK_MOD_OPTIONS.RESIST:
                return executeApplyDamageToTargets(
                    canvas.tokens.controlled,
                    moveData,
                    !moveData.isCrit ? moveData.regDamage : moveData.critDamage,
                    { isResist: true, damageReduction: dr }
                )
            case ATTACK_MOD_OPTIONS.FLAT:
                return executeApplyDamageToTargets(
                    ...
                    moveData,
                    !moveData.isCrit ? moveData.regDamage : moveData.critDamage,
                    { isFlat: true, damageReduction: dr }
                )
        }
        return
    }
...
async function executeApplyDamageToTargets(
    targets,
    data,
    damage,
    { isFlat, isResist, isWeak, damageReduction, msgId } = {
        isFlat: false,
        isResist: false,
        isWeak: false,
    }
    ...
    return await game.ptu.utils.api.gm.applyDamage(
        targets,
        damage,
        data.type,
        data.category,
...
export async function displayAppliedDamageToTargets(appliedDamage) {
    let messageData = {
        ...
        content: await renderTemplate(
            `/systems/${SystemPaths.systemId()}/templates/chat/automation/applied-damage.hbs`,
            appliedDamage
        ),
...
export async function undoDamageToTargets(event) {
    ...
    const actor =
        document instanceof Token || document instanceof TokenDocument
            ? document.actor
            : document
    if (!actor) return

    log(
        `FVTT PTU | Undoing ${data.injuries} injuries and ${data.damage} damage to ${actor.name} - Old Injuries: ${data.oldInjuries} - Old HP: ${data.oldHp} - Old Temp: ${data.oldTempHp}`
    )
    await actor.update({
        'system.health.injuries': data.oldInjuries,
        'system.health.value': data.oldHp,
        'system.tempHp.value': data.oldTempHp,
        'system.tempHp.max': data.oldTempHp,
    })

    if (data.msgId) {
        await updateApplicatorHtml(
...
export async function applyEffectsToTargets(event) {
    ...
    if (event.hasDataset) dataset = event
        ...
        dataset = event.currentTarget.dataset.moveUuid
            ? event.currentTarget.dataset
            : event.currentTarget.parentElement.parentElement.dataset
    }
    ...
    if (target === 'many') {
        ...
        messageHtml
            .find('.mon-list')
            .filter((k, i) => !i.className.includes('disabled'))
            .each((k, i) => {
        ...
        if (document) {
            if (
                document instanceof Token ||
                document instanceof TokenDocument ||
    ...
    results.push(
        await applyResult(
            targets,
            0,
            messageHtml[0]?.dataset?.messageId ?? dataset.messageId
        )
    )
    ...
    async function applyResult(targets, damage, messageId) {
        ...
        const effectData = await beforeDamage(attacks, targets, move, {
            roll: acRoll,
            msgId: messageId,
        })
        ...
        if (effectData.length > 0) {
            const content = await renderTemplate(
                `/systems/${SystemPaths.systemId()}/templates/chat/automation/applied-effect.hbs`,
                { effectData }
            )
            let messageData = {
                user: game.user.id,
    ...
    async function beforeDamage(attacks, targets, move, options) {
        ...
        for (const target of targets) {
            ...
            if (system.passives?.hit?.length > 0) {
                ...
                for (const passive of passives.filter(
                    (a) =>
                        a.automation.timing ==
                        CONFIG.PTUAutomation.Timing.BEFORE_DAMAGE
                )) {
                    modifiers.push(
                        ...(await ApplyAutomation(move, passive.automation, {
                            targets: [target],
                            roll: options.roll,
                            passiveName: passive.itemName,
                        }))
                    )
                }
            }
        ...
        if (move.system.automation?.length > 0) {
            ...
            for (const automation of move.system.automation.filter(
                (a) => a.timing == CONFIG.PTUAutomation.Timing.BEFORE_DAMAGE
            )) {
                modifiers.push(
                    ...(await ApplyAutomation(move, automation, {
                        targets,
                        roll: options.roll,
                    }))
                )
                    ...
                    switch (modifier.modifier.type) {
                        case CONFIG.PTUAutomation.Modifiers.DAMAGE: {
                            attack.damage =
                                ...
                                Number(modifier.modifier.value)
                            break
                        }
                        case CONFIG.PTUAutomation.Modifiers.EFFECTIVENESS: {
                            attack.effectiveness =
                                Number(attack.effectiveness) +
                                Number(modifier.modifier.value)
                            break
                        }
                    }
...
export async function applyDamageAndEffectsToTargets(event) {
            ...
            .each((k, i) => {
                ...
                if (actor && i.dataset.hit == 'true') {
                    if (
                        i.dataset.crit == 'hit' ||
                        i.dataset.crit == 'crit' ||
                        i.dataset.crit == 'double-hit'
                    )
                        critTargets.push(actor)
                    else targets.push(actor)
                }
            })
    } else {
        const document = await fromUuid(target)
    ...
    if (messageHtml?.length != 0)
        await updateApplicatorHtml(
            messageHtml,
            targets.concat(critTargets).map((t) => (t.document ?? t).uuid),
            mode,
            true
        )
    ...
    async function applyResult(targets, damage, messageId) {
        ...
        // Step 2: perform beforeDamage effects
        const { modifiedAttacks, effects } = await beforeDamage(
            attacks,
            targets,
            move,
            { roll: acRoll, msgId: messageId }
        )

        // Step 3: Resolve attacks and apply damage
        const results = await game.ptu.utils.api.gm.applyAttacks(
            modifiedAttacks,
            { msgId: messageId, moveName: move.name, effects }
        )
...
async function ApplyAutomation(source, automation, options) {
    ...
    const filteredTargets = filterTargetsByCondition(targets, automation, {
        roll: options.roll,
        source,
    })
        ...
        for (const target of automation.targets) {
            ...
            switch (target) {
                case CONFIG.PTUAutomation.Target.TARGET: {
                    const objects =
                        options.targets instanceof Array
                            ? options.targets
                            : [options.targets]
                    for (const object of objects) {
                        ...
                        if (
                            object instanceof TokenDocument ||
                        ...
                        if (typeof object === 'string') {
                            ...
                            if (objectFromUuid) {
                                if (objectFromUuid instanceof Token)
                                    targets.push(objectFromUuid.document)
                                if (
                                    objectFromUuid instanceof TokenDocument ||
                                    objectFromUuid instanceof Actor
                                )
                                    targets.push(objectFromUuid)
                                continue
                            }
                            ...
                            if (game.user.targets.ids[0]) {
                                const tokenFromTarget = canvas.tokens.get(
                                    game.user.targets.ids[0]
                                )
                                if (tokenFromTarget) {
                ...
                switch (condition.type) {
                    case CONFIG.PTUAutomation.Condition.ATTACK_ROLL: {
                        ...
                        switch (condition.rangeIncrease) {
                            case CONFIG.PTUAutomation.RangeIncreases.NONE:
                                break
                            case CONFIG.PTUAutomation.RangeIncreases
                                .EFFECT_RANGE: {
                                if (!options.source) break
                                let source = options.source
                                if (
                                    source instanceof Token ||
                                    source instanceof TokenDocument
                                )
                                    source = source.actor
                                ...
                                const r =
                                    source?.system?.modifiers?.effectRange
                                        ?.total
                                if (!r) break
                                range += `+(-1*${r})`
                                break
                        ...
                        //FIXME remove eval. What the fuck is the fucking shit fucking code. god fucking damn it. What a fucking stupid use of eval.
                        //TODO replace with below code
                        // if (compareValues(roll, operator, value + range)) passedConditions++;
                        ...
                        if (eval(`${roll} ${operator} (${value} ${range})`))
                            passedConditions++
                        break
                    }
                    case CONFIG.PTUAutomation.Condition.EFFECTIVENESS: {
                        //TODO: This does not take into account any prior changes to advantage before this check.
                        ...
                        const effectiveness =
                            actor.system.effectiveness.All[
                                Handlebars.helpers.capitalizeFirst(type)
                            ]
                        ...
                        if (
                            eval(
                                `${effectiveness} ${condition.operator} ${condition.value}`
                            )
                        )
                            passedConditions++
                        break
                    ...
                    case CONFIG.PTUAutomation.Condition.ITEM_TYPE: {
                        ...
                        const { operator, value } = condition
                        if (eval(`target.type ${operator} ${value}`))
                            passedConditions++
                        break
                    }
                    case CONFIG.PTUAutomation.Condition.MOVE_TYPE: {
                        ...
                        const { operator, value } = condition
                        if (eval(`target.system.type ${operator} ${value}`))
                            passedConditions++
                        break
                    }
                }
        ...
        for (const target of targets) {
                ...
                switch (effect.type) {
                    case CONFIG.PTUAutomation.Effect.ADD_EFFECT: {
                        ...
                        if (typeof effect.value === 'string') {
                            const effectData = CONFIG.statusEffects.find(
                                (e) => e.id === effect.value
                            )
                            ...
                            if (
                                ...
                                const ae = target.actor.effects.find(
                                    (e) => e.label == effectData.label
                                )
                                if (ae) continue
                    ...
                    case CONFIG.PTUAutomation.Effect.ADD_DAMAGE: {
                        ...
                        if (
                            localTarget instanceof TokenDocument ||
                            localTarget instanceof Actor
                        ) {
            ...
            for (const effect of updates.effects.toggle) {
                ...
                if (target instanceof TokenDocument)
                    await target.toggleActiveEffect(effect)
            }
...
export async function newApplyDamageToTargets(event) {
    ...
    if (event.hasDataset) dataset = event
        ...
        dataset = event.currentTarget.dataset.moveName
            ? event.currentTarget.dataset
            : event.currentTarget.parentElement.parentElement.dataset
    }
    ...
    if (moveData.target === 'many') {
        const messageHtml = $(event.currentTarget).closest(
        ...
        if (targets.length != 0) {
            ...
            r.push(
                await applyResult(
                    targets,
                    moveData.damage,
                    messageHtml[0].dataset.messageId
                )
            )
        }
        ...
        await updateApplicatorHtml(
            messageHtml,
            targets.concat(critTargets).map((t) => t.id),
            moveData.mode,
            true
        )
    ...
    moveData.target = canvas.tokens.get(moveData.target)
        ? [canvas.tokens.get(moveData.target)]
        : false
    ...
    function applyResult(targets, damage, messageId) {
        switch (moveData.mode) {
            case 'full':
                return executeApplyDamageToTargets(targets, moveData, damage, {
                    damageReduction: dr,
                    msgId: messageId,
                })
            case 'weak':
            ...
            case 'half':
                return executeApplyDamageToTargets(
                    ...
                    moveData,
                    Math.max(1, damage / 2),
                    { damageReduction: dr, msgId: messageId }
                )
            case 'flat':
                return executeApplyDamageToTargets(targets, moveData, damage, {
                    isFlat: true,
...
export async function handleApplicatorItem(event) {
    ...
    if (currentTarget.className.includes('applicators')) {
        ...
        const data = {
            ...
            crit:
                parent.dataset.crit == 'crit' ||
                parent.dataset.crit == 'double-hit',
            hasDataset: true,
            messageId,
        }
    ...
    await game.ptu.utils.api.gm.chatMessageUpdate(message, {
        content: newContent,
    })
}

async function updateApplicatorHtml(
    root,
    targetIds,
    mode,
    ...
    const applicatorHTML = {
        normal: `<i class="tooltip fas fa-circle" data-mode="full">
                <span class="tooltip-content">Apply Normal</span>
            </i>`,
        half: `<i class="tooltip fas fa-adjust" data-mode="half">
                <span class="tooltip-content">Apply Half</span>
            </i>`,
        weak: `<i class="tooltip fas fa-minus-square" data-mode="weak">
                <span class="tooltip-content">Apply Resist Less</span>
            </i>`,
        resist: `<i class="tooltip fas fa-plus-square" data-mode="resist">
                <span class="tooltip-content">Apply Resist More</span>
            </i>`,
        flat: `<i class="tooltip fas fa-stop" data-mode="flat">
    ...
    for (const target of targetIds) {
        ...
        if (undo) {
            ...
            monList
                .children('.applicators')
                .html(
                    applicatorHTML.normal +
                        '\n' +
                        applicatorHTML.half +
                        '\n' +
                        applicatorHTML.weak +
...
export async function TakeAction(actor, { actionType, actionSubType, label }) {
    ...
    if (
        game.settings.get('ptu', 'useExtraActionHomebrew') &&
        !supportAction &&
        actionSubType != ActionSubTypes.PHYSICAL &&
        actionSubType != ActionSubTypes.SPECIAL
    ) {
        addChanges(changesToApply, {
            actionType,
            actionSubType,
            isSupport: true,
        })
    ...
    function addChanges(
        changesToApply,
        { actionType, actionSubType, isSupport }
    ) {
        if (isSupport) {
            changesToApply.push({
                key: 'flags.ptu.actions_taken.support',
                value: true,
        ...
        if (actionType && actionType != ActionTypes.DEFAULT) {
            if (actionType == ActionTypes.FULL) {
                changesToApply.push({
                    key: `flags.ptu.actions_taken.shift`,
                    value: true,
                    mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                    priority: 50,
                })
                changesToApply.push({
                    key: `flags.ptu.actions_taken.standard`,
                    value: true,
                    mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                    priority: 50,
                })
                ...
                changesToApply.push({
                    key: `flags.ptu.actions_taken.${actionType.toLowerCase()}`,
                    value: true,
                    mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                    priority: 50,
                })
        ...
        if (actionSubType && actionSubType != ActionSubTypes.DEFAULT) {
            changesToApply.push({
                key: `flags.ptu.actions_taken.attacked.${actionSubType.toLowerCase()}`,
                value: true,
                mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                priority: 50,
            })
...
export async function ApplyInjuries(
    target_actor,
    final_effective_damage,
    damage_type = 'Untyped'
) {
    ...
    const hitPoints50Pct = targetHealthMax * 0.5
    const hitPoints0Pct = 0
    const hitPointsNegative50Pct = targetHealthMax * -0.5
    const hitPointsNegative100Pct = targetHealthMax * -1.0
    const hitPointsNegative150Pct = targetHealthMax * -1.5
    const hitPointsNegative200Pct = targetHealthMax * -2.0
    ...
    if (game.settings.get('ptu', 'autoApplyInjuries') == 'true') {
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPoints50Pct &&
            targetHealthCurrent - final_effective_damage <= hitPoints50Pct
        ) {
            injuryCount++
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPoints0Pct &&
            targetHealthCurrent - final_effective_damage <= hitPoints0Pct
        ) {
            injuryCount++
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPointsNegative50Pct &&
            targetHealthCurrent - final_effective_damage <=
                hitPointsNegative50Pct
        ) {
            injuryCount++
            // await game.PTUMoveMaster.chatMessage(target_actor, target_actor.name + " was damaged to below the -50% health threshold and sustains an injury!");
        }
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPointsNegative100Pct &&
            targetHealthCurrent - final_effective_damage <=
                hitPointsNegative100Pct
        ) {
            injuryCount++
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPointsNegative150Pct &&
            targetHealthCurrent - final_effective_damage <=
                hitPointsNegative150Pct
        ) {
            injuryCount++
        ...
        if (
            final_effective_damage >= 1 &&
            targetHealthCurrent > hitPointsNegative200Pct &&
            targetHealthCurrent - final_effective_damage <=
                hitPointsNegative200Pct
        ) {
            injuryCount++
        ...
        const actualInjuries = Math.min(
            Number(currentInjuries + injuryCount),
            10
        )
        if (currentInjuries + injuryCount > 10)
            injuryCount -= (currentInjuries + injuryCount) % 10
