...
export default class MenuComponent extends Component {
    ...
    async render() {
        ...
        switch (this.state.menuOption) {
            case 'struggle':
                output += await renderTemplate(
                    `/systems/ptu/templates/sidebar/components/menu-component.hbs`,
                    {
                        menu: 'struggle',
                ...
                break
            case 'pokeball':
                if (this.state.actor.type == 'pokemon')
                    return this.store.dispatch('changeMenuOption', 'none')

                output += await renderTemplate(
                    `/systems/ptu/templates/sidebar/components/menu-component.hbs`,
                    {
                        menu: 'pokeball',
                        ball: await this._getTrainerPokeballArray(
                            this.state.actor
                        ),
                    }
                )
            ...
            case 'maneuver':
                ...
                output += await renderTemplate(
                    ...
                    {
                        menu: 'maneuver',
                        maneuvers: sortedManeuverObjects,
                    }
                )
                break
            default:
                ...
                switch (type) {
                    ...
                    case 'pokeball':
                        if (this.state.menuOption == type) {
                            ...
                            AudioHelper.play(
                                {
                                    src: pokeball_sound_paths['menu_close'],
                                    volume: 0.8,
                                    autoplay: true,
                                    loop: false,
                                },
                            ...
                            AudioHelper.play(
                                {
                                    src: pokeball_sound_paths['menu_open'],
                                    volume: 0.8,
                                    autoplay: true,
                                    loop: false,
                                },
        ...
        this.element
            ...
            .on('click', async (event) => {
                ...
                if (
                    game.settings.get('ptu', 'pokeball-prompts') == 1 ||
                    game.settings.get('ptu', 'pokeball-prompts') == 3
                ) {
                    if (game.ptu.utils.api.gm._isMainGM()) allowed = 'true'
                    //ask GM to confirm they want to allow player to throw a pokeball?
                    else
                        allowed =
                            await game.ptu.utils.api.gm.throwPokeballRequest(
                                owner,
                                target,
                                { timeout: 30000 }
                            )
                ...
                if (allowed == 'true') {
                    game.ptu.utils.throwPokeball(
                        owner,
                        target,
                        owner?.items.get(entityId)
                    )
                } else {
                    ui.notifications.error(
                        'GM prevented you from throwing a pokeball at this pokemon.'
                    )
                    return
                }
            })
        ...
        this.element
            .children('#menu-content')
            .children('.struggle-row')
            .children('.movemaster-button[data-struggle-id]')
            .on('mousedown', (event) => {
                // Handle on move click here, for now just log that button is clicked
                const move = this.state.actor.items.get(
                    event.currentTarget?.dataset?.struggleId
                )
                switch (event.which) {
                    case 3: // Right click
                        sendMoveMessage({
        ...
        this.element
            .children('#menu-content')
            .children('.maneuvers-row')
            .children('.movemaster-button[data-maneuver-name]')
            .on('click', (event) => {
                this._sendManeuverToChat(
                    event.currentTarget.dataset.maneuverName
                )
            })
    ...
    async _getStruggles(actor) {
        ...
        actor.itemTypes.move.forEach((m) => {
            if (
                m.system.isStruggle ||
                (m.name.includes('Struggle') &&
                    m.system.type != '' &&
                    m.system.type != '--')
            )
                currentStruggles.push(m)
        })
        ...
        for (const struggle of foundStruggles) {
            ...
            if (struggle != 'telekinetic') {
                const move = currentStruggles.find(
                    (m) =>
                        m.system.type == type && m.system.category == 'Physical'
                )
                if (!move)
                    strugglesToCreate.push(
                        this._prepareNewStruggle(actor, type, isTelekinetic)
                    )
            ...
            const move = currentStruggles.find(
                (m) => m.system.type == type && m.system.category == 'Special'
            )
            if (!move)
                strugglesToCreate.push(
                    this._prepareNewStruggle(actor, type, isTelekinetic, false)
                )
            else strugglesToDisplay.push(move)
        }

        if (strugglesToCreate.length > 0)
            strugglesToDisplay.push(
    ...
    _prepareNewStruggle(actor, type, isTelekinetic, isPhysical = true) {
        ...
        return {
            ...
            data: {
                ...
                range:
                    (isTelekinetic
                        ? actor.system.skills.willpower.value.total
                        : 'Melee') + ', 1 Target',
                frequency: 'At-Will',
                effect: '--',
            },
    ...
    async _getTrainerPokeballArray(actor) {
        return actor.items
            .filter(
                (item) =>
                    item.type == 'item' &&
                    item.system.category.toLowerCase() == 'pokeballs'
            )
            .map((item) => {
                return {
                    name: item.name,
    ...
    async _sendManeuverToChat(maneuverName) {
        const maneuver = await game.packs
            .get('ptu.maneuvers')
            .find((m) => m.name.toLowerCase() == maneuverName.toLowerCase())
        ...
        messageData.content = await renderTemplate(
            `/systems/ptu/templates/chat/maneuver.hbs`,
            messageData
        )
