...
import { warn, debug, log } from "../../main";
...
export class PTUActor extends Actor {
    ...
    this.origins = mergeObject(
      {
        system: {
          levelUpPoints: [
            ...
            {
              label: "Level",
              change: { type: CONST.ACTIVE_EFFECT_MODES.ADD, value: actorSystem.level.current },
            },
            {
              label: "HP Stat",
              change: { type: CONST.ACTIVE_EFFECT_MODES.ADD, value: -actorSystem.stats.hp.levelUp },
            },
            {
              label: "ATK Stat",
              change: {
            ...
            physical: [
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: Math.min(Math.floor(actorSystem.stats.def.total / 5), 6),
                },
              },
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: actorSystem.modifiers.evasion.physical.total,
                },
              },
            ],
            special: [
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: Math.min(Math.floor(actorSystem.stats.spd.total / 5), 6),
                },
              },
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: actorSystem.modifiers.evasion.special.total,
                },
              },
            ],
            speed: [
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: Math.min(Math.floor(actorSystem.stats.spe.total / 5), 6),
                },
              },
              {
                ...
                change: {
                  type: CONST.ACTIVE_EFFECT_MODES.ADD,
                  value: actorSystem.modifiers.evasion.speed.total,
                },
              },
            ],
          },
              ...
              stage: {
                mod: [
                  this.flags.ptu?.is_poisoned
                    ? {
                        label: "Poisoned",
                        change: { type: CONST.ACTIVE_EFFECT_MODES.ADD, value: -2 },
                      }
                    : undefined,
  ...
  async prepareDerivedData() {
    ...
    if (actorData.type === "character" && actorSystem.skills.intimidation) {
      ...
      setTimeout(
        () => this.update({ "data.skills": skills, "data.skills.-=intimidation": null }),
        1000
      );
    }
  }
  ...
  static async create(data, options = {}) {
    ...
    let form = new game.ptu.config.Ui.PokemonCharacterMancer.documentClass(actor, {
  ...
  _prepareCharacterData() {
    ...
    const leftoverLevelUpPoints =
      data.levelUpPoints - Object.values(data.stats).reduce((a, v) => v.levelUp + a, 0);
      ...
      data.level.current -
        Math.max(
          ...
          Math.clamped(
            ...
            leftoverLevelUpPoints - data.modifiers.statPoints.total ?? 0
          )
        )
    );
          ...
          data.stats,
          {
            twistedPower:
              actorData.items.find(
                (x) => x.name.toLowerCase().replace("[playtest]") == "twisted power"
              ) != null,
          },
          data.nature?.value,
          true
  ...
  _preparePokemonData() {
    ...
    data.level.percent = Math.round(
      ((data.level.exp - game.ptu.data.levelProgression[data.level.current]) /
        (data.level.expTillNextLevel - game.ptu.data.levelProgression[data.level.current])) *
        100
    );
    ...
    if (data.modifiers?.typeOverwrite) {
      ...
      for (const type of splitTypes) {
        if (game.ptu.data.TypeEffectiveness[Handlebars.helpers.capitalizeFirst(type.toLowerCase())])
          types.push(type);
      }
    }
    ...
    data.tp.pep.value = actorData.items.filter(
      (x) => x.type == "enhancement" && x.system.origin?.toLowerCase() != "pusher"
    ).length;
  ...
  async rollMove(move, options = { moveId, bonusDamage, targets, APBonus }) {
    ...
    if (game.combat?.id && move.getFlag("ptu", "lastEncounterUsed") != game.combat?.id)
  ...
  async _performFullAttack(moveData, token, { bonusDamage, targets, moveName, APBonus }) {
    ...
    moveData.doubleStrike = {
      is:
        (moveData.range?.toLowerCase().includes("double strike") ?? false) ||
        (moveData.range?.toLowerCase().includes("doublestrike") ?? false),
      hit1: undefined,
      hit2: undefined,
    };
    ...
    bonusDamage +=
      moveData.category.toLowerCase() == "physical"
        ? this.system.modifiers?.damageBonus?.physical?.total + this.system.stats.atk.total
        : moveData.category.toLowerCase() == "special"
        ? this.system.modifiers?.damageBonus?.special?.total + this.system.stats.spa.total
        : 0;
    ...
    if (moveData.doubleStrike.is === true) {
      ...
      critType =
        ...
        moveData.doubleStrike.hit2.crit == CritOptions.CRIT_HIT
          ...
          : moveData.doubleStrike.hit1.crit == CritOptions.CRIT_MISS &&
            moveData.doubleStrike.hit2.crit == CritOptions.CRIT_MISS
    ...
    for (const attackInfo of Object.values(attacksData)) {
      if (moveData.doubleStrike.is === true) {
        moveData.doubleStrike.hit1.hit =
          moveData.doubleStrike.hit1.roll.total >= attackInfo.target.evasion.value;
        moveData.doubleStrike.hit2.hit =
          moveData.doubleStrike.hit2.roll.total >= attackInfo.target.evasion.value;
        attackInfo.isHit = alwaysHits
          ? true
          : critType == CritOptions.CRIT_MISS
          ? false
          : moveData.doubleStrike.hit1.hit || moveData.doubleStrike.hit2.hit;
      } else {
        attackInfo.isHit = alwaysHits
          ? true
          : critType == CritOptions.CRIT_MISS
    ...
    await game.ptu.utils.combat.takeAction(this, {
      ...
      label: `R${game.combat?.current?.round ?? 0} - T${
          ...
          function isInRange() {
            switch (true) {
              ...
              case moveRange.includes("burst"):
                return moveRange
                  .slice(moveRange.indexOf("burst") + "burst".length)
                  .split(",")[0]
                  .trim();
              case moveRange.includes("cone"):
                return moveRange
                  .slice(moveRange.indexOf("cone") + "cone".length)
                  .split(",")[0]
                  .trim();
              case moveRange.includes("line"):
                return moveRange
                  .slice(moveRange.indexOf("line") + "line".length)
                  .split(",")[0]
                  .trim();
              case moveRange.includes("close blast"):
                return moveRange
                  .slice(moveRange.indexOf("close blast") + "close blast".length)
                  .split(",")[0]
                  .trim();
              case moveRange.includes("ranged blast"):
                return (
                  ...
                  Number(
                    moveRange
                      .slice(moveRange.indexOf("ranged blast") + "ranged blast".length)
                      .split(",")[0]
                      .trim()
                  )
                );
    ...
    async function calculateTotalDamageBonus(
      ...
      if (moveData.damageBonus > 0) {
        ...
        modifierTexts.push(
          `Including ${moveData.damageBonus >= 0 ? "+" : ""}${
            moveData.damageBonus
          } damage from [Move Damage Modifier Field]`
        );
      }
        ...
        await Dialog.confirm({
          ...
          yes: async (html) => {
            ...
            const bonus = !isNaN(Number(bonusTxt))
              ? Number(bonusTxt)
              : parseInt((await new Roll(bonusTxt).roll({ async: true })).total);
            if (!isNaN(bonus)) {
              total += bonus;
              modifierTexts.push(
  ...
  _calculateDamageRoll(moveData, critType, damageBonus, abilityBonuses, actorData, targetData) {
    ...
    if (
      ...
      (moveData.doubleStrike.is === true || moveData.fiveStrike.is === true || db <= 6)
    ) {
      dbBonus += 2;
    }
    ...
    if (
      this.system.typing &&
      (moveData.type == this.system.typing[0] || moveData.type == this.system.typing[1])
    ) {
    ...
    if (moveData.name.toLowerCase().includes("stored power")) {
      const dbBonusFromStages = Math.min(
        20 - db,
        ((actorData.stats.atk.stage.value + actorData.stats.atk.stage.mod < 0
          ? 0
          : actorData.stats.atk.stage.value + actorData.stats.atk.stage.mod) +
          (actorData.stats.spa.stage.value + actorData.stats.spa.stage.mod < 0
            ? 0
            : actorData.stats.spa.stage.value + actorData.stats.spa.stage.mod) +
          (actorData.stats.def.stage.value + actorData.stats.def.stage.mod < 0
            ? 0
            : actorData.stats.def.stage.value + actorData.stats.def.stage.mod) +
          (actorData.stats.spd.stage.value + actorData.stats.spd.stage.mod < 0
            ? 0
            : actorData.stats.spd.stage.value + actorData.stats.spd.stage.mod) +
          (actorData.stats.spe.stage.value + actorData.stats.spe.stage.mod < 0
            ? 0
            : actorData.stats.spe.stage.value + actorData.stats.spe.stage.mod)) *
          2
      );
    ...
    if (moveData.name.toLowerCase().includes("punishment")) {
      const dbBonusFromStages = Math.min(
        12 - db,
        (targetData?.stats?.atk.stage.value + targetData?.stats?.atk.stage.mod < 0
          ? 0
          : targetData?.stats?.atk.stage.value + targetData?.stats?.atk.stage.mod) +
          (targetData?.stats?.spa.stage.value + targetData?.stats?.spa.stage.mod < 0
            ? 0
            : targetData?.stats?.spa.stage.value + targetData?.stats?.spa.stage.mod) +
          (targetData?.stats?.def.stage.value + targetData?.stats?.def.stage.mod < 0
            ? 0
            : targetData?.stats?.def.stage.value + targetData?.stats?.def.stage.mod) +
          (targetData?.stats?.spd.stage.value + targetData?.stats?.spd.stage.mod < 0
            ? 0
            : targetData?.stats?.spd.stage.value + targetData?.stats?.spd.stage.mod) +
          (targetData?.stats?.spe.stage.value + targetData?.stats?.spe.stage.mod < 0
            ? 0
            : targetData?.stats?.spe.stage.value + targetData?.stats?.spe.stage.mod)
      );
  ...
  async _commandCheck(trainerActor) {
    ...
    for (const item of trainerActor.items) {
      ...
      if (
        trainerActor.system.skills[ALTERNATE_COMMAND_SKILL_FEATURES[item.name]].value.total +
          trainerActor.system.skills[ALTERNATE_COMMAND_SKILL_FEATURES[item.name]].modifier.total *
            0.1 >
...
export function GetSpeciesData(species) {
  ...
  if (species) {
    ...
    if (parseInt(species)) {
      ...
      if (!preJson) {
        preJson = game.ptu.data.pokemonData.find(
          (x) => x._id?.toLowerCase() === species.toLowerCase() + "-normal"
        );
        if (!preJson) {
          preJson = game.ptu.data.pokemonData.find(
            (x) => x._id?.toLowerCase() === species.toLowerCase() + "-male"
          );
          if (!preJson) return null;
        }
      }
