...
import {
    CalculatePokemonCapabilities,
    CalculateTrainerCapabilities,
} from './calculations/capability-calculator.js'
import { CalculateSkills } from './calculations/skills-calculator.js'
import {
    CalcBaseStats,
    CalculateStatTotal,
    CalculatePTStatTotal,
    CalculatePoisonedCondition,
} from './calculations/stats-calculator.js'
...
} from '../combat/effects/move_animations.js'
import { PlayMoveSounds } from '../combat/effects/move_sounds.js'
import {
    ActionTypes,
    FiveStrikeHitsDictionary,
...
export class PTUActor extends Actor {
    ...
    prepareData() {
        ...
        if (parseInt(game.version.split('.')[1]) <= 6) {
            ...
            if (actorData.type === 'character')
                this._prepareCharacterData(actorSystem)
            if (actorData.type === 'pokemon')
                this._preparePokemonData(actorSystem)
        }
        ...
        this.origins = mergeObject(
            {
                system: {
                    levelUpPoints: [
                        {
                            ...
                            change: {
                                ...
                                value: actorData.type === 'character' ? 9 : 10,
                            },
                        },
                        {
                            label: 'Level',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: actorSystem.level.current,
                            },
                        },
                        {
                            label: 'HP Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.hp.levelUp,
                            },
                        },
                        {
                            label: 'ATK Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.atk.levelUp,
                            },
                        },
                        {
                            label: 'DEF Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.def.levelUp,
                            },
                        },
                        {
                            label: 'SPA Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.spa.levelUp,
                            },
                        },
                        {
                            label: 'SPD Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.spd.levelUp,
                            },
                        },
                        {
                            label: 'SPE Stat',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: -actorSystem.stats.spe.levelUp,
                            },
                        },
                        {
                            label: 'Stat Point Modifier',
                            change: {
                                type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                value: actorSystem.modifiers.statPoints.total,
                            },
                        },
                    ],
                    evasion: {
                        physical: [
                            {
                                label: 'DEF Stat / 5 (max 6)',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: Math.min(
                                        Math.floor(
                                            actorSystem.stats.def.total / 5
                                        ),
                                        6
                                    ),
                                },
                            ...
                            {
                                label: 'Physical Evasion Mod',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: actorSystem.modifiers.evasion
                                        .physical.total,
                                },
                            },
                        ],
                        special: [
                            {
                                label: 'SPD Stat / 5 (max 6)',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: Math.min(
                                        Math.floor(
                                            actorSystem.stats.spd.total / 5
                                        ),
                                        6
                                    ),
                                },
                            ...
                            {
                                label: 'Special Evasion Mod',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: actorSystem.modifiers.evasion.special
                                        .total,
                                },
                            },
                        ],
                        speed: [
                            {
                                label: 'SPE Stat / 5 (max 6)',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: Math.min(
                                        Math.floor(
                                            actorSystem.stats.spe.total / 5
                                        ),
                                        6
                                    ),
                                },
                            ...
                            {
                                label: 'Speed Evasion Mod',
                                change: {
                                    type: CONST.ACTIVE_EFFECT_MODES.ADD,
                                    value: actorSystem.modifiers.evasion.speed
                                        .total,
                                },
                            },
                        ],
                    ...
                    skills:
                        actorSystem.modifiers.skillBonus.total > 0
                            ? Object.keys(actorSystem.skills)
                                  .map((skill) => {
                                      return {
                                          [skill]: {
                                              modifier: {
        ...
        if (
            game.ptu.forms.sidebar?.store?.state?.targetedActors.includes(
                this.id
            )
        )
    ...
    applyActiveEffects(doBaseData = true) {
        ...
        const effects = Array.from(this.effects).concat(
            this.items
            ...
            return changes.concat(
                e.changes
                    .map((c) => {
                        ...
                        if (c.key.startsWith('data.'))
                            c.key = c.key.replace('data.', 'system.')
        ...
        for (let change of changes) {
            ...
            if (result !== null) {
                ...
                origins[change.key].push({
                    label: change.effect.label,
                    change: { type: change.mode, value: change.value },
                })
            }
        }
    ...
    async modifyTokenAttribute(
        ...
        if (isBar) {
            if (attribute == 'health') {
                ...
                debug('Updating Character HP with args:', this, {
  ...
  _preparePokemonData() {
    ...
    for(const item of actorData.items) {
      if(item.system.automation?.length > 0) {
        ...
        // Update data structures.
        {
            ...
            if (!isNaN(actorSystem.skills.acrobatics.value)) {
                ...
                for (let [key, skill] of Object.entries(skills)) {
                    ...
                    skill['modifier'] = {
                        value: !isNaN(Number(skill.modifier))
                            ? skill.modifier
                            : 2,
                        mod: 0,
                    }
            ...
            if (isNaN(actorSystem.modifiers.initiative.value)) {
                ...
                for (let [key, value] of Object.entries(modifiers)) {
                    if (
                        key == 'hardened' ||
                        key == 'flinch_count' ||
                        key == 'immuneToEffectDamage' ||
                        key == 'skillBonus'
                    )
                        continue
                    if (key == 'evasion') {
                        for (let [evasion, actualValue] of Object.entries(
                            value
                        )) {
                            modifiers[key][evasion] = {
                                value: actualValue,
        ...
        if (actorData.type === 'character' && actorSystem.skills.intimidation) {
            ...
            delete skills.intimidation
            log(
                `PC ${this.name} with old 'Intimidation' skill found, updating to 'Intimidate' skill.`
            )
            setTimeout(
                () =>
                    this.update({
                        'data.skills': skills,
                        'data.skills.-=intimidation': null,
                    }),
                1000
            )
        }
    ...
    static async create(data, options = {}) {
        ...
        let form = new game.ptu.config.Ui.PokemonCharacterMancer.documentClass(
            actor,
            { submitOnChange: false, submitOnClose: true }
        )
        form.render(true)
    ...
    _prepareCharacterData() {
        ...
        const dexExpEnabled =
            'true' == game.settings.get('ptu', 'useDexExp') ?? false
        ...
        for (let [key, skill] of Object.entries(data.skills)) {
            skill['rank'] = this._getRank(skill['value']['value'])
            skill['value']['total'] =
                skill['value']['value'] + skill['value']['mod']
            skill['modifier']['total'] =
                skill['modifier']['value'] + skill['modifier']['mod']
        }
        ...
        for (let [key, mod] of Object.entries(data.modifiers)) {
            ...
            if (
                ...
                for (let [subkey, value] of Object.entries(mod)) {
                    data.modifiers[key][subkey]['total'] =
                        (value['value'] ?? 0) + (value['mod'] ?? 0)
                }
                continue
            } else {
                data.modifiers[key]['total'] =
                    (mod['value'] ?? 0) + (mod['mod'] ?? 0)
            }
        }
        ...
        if (dexExpEnabled) {
            ...
            data.level.current =
                ...
                50
                    ? 50
                    : Math.trunc(
                          (data.level.dexexp + data.level.miscexp) / 10
                      ) + 1
        } else {
            data.level.current =
        ...
        data.levelUpPoints =
            data.level.current + data.modifiers.statPoints.total + 9
        data.stats = CalculatePoisonedCondition(
            duplicate(data.stats),
            actorData.flags?.ptu
        )

        const leftoverLevelUpPoints =
            data.levelUpPoints -
            Object.values(data.stats).reduce((a, v) => v.levelUp + a, 0)
        const actualLevel = Math.max(
            ...
            data.level.current -
                Math.max(
                    ...
                    Math.clamped(
                        0,
                        leftoverLevelUpPoints,
                        leftoverLevelUpPoints -
                            data.modifiers.statPoints.total ?? 0
                    )
                )
        )
        ...
        data.stats.hp.base = 10
        data.stats.hp.value =
            data.modifiers.baseStats.hp.total + data.stats.hp.base
        data.stats.atk.base = 5
        data.stats.atk.value =
            data.modifiers.baseStats.atk.total + data.stats.atk.base
        data.stats.def.base = 5
        data.stats.def.value =
            data.modifiers.baseStats.def.total + data.stats.def.base
        data.stats.spa.base = 5
        data.stats.spa.value =
            data.modifiers.baseStats.spa.total + data.stats.spa.base
        data.stats.spd.base = 5
        data.stats.spd.value =
            data.modifiers.baseStats.spd.total + data.stats.spd.base
        data.stats.spe.base = 5
        data.stats.spe.value =
            data.modifiers.baseStats.spe.total + data.stats.spe.base
        ...
        var result = game.settings.get('ptu', 'playtestStats')
                  ...
                  data.stats,
                  {
                      twistedPower:
                          actorData.items.find(
                              (x) =>
                                  x.name.toLowerCase().replace('[playtest]') ==
                                  'twisted power'
                          ) != null,
                  },
                  data.nature?.value,
            ...
            : CalculateStatTotal(data.levelUpPoints, data.stats, {
                  twistedPower:
                      actorData.items.find(
                          (x) =>
                              x.name.toLowerCase().replace('[playtest]') ==
        ...
        data.health.total =
            10 + data.level.current * 2 + data.stats.hp.total * 3
        data.health.max =
            data.health.injuries > 0
                ? Math.trunc(
                      data.health.total *
                          (1 -
                              (data.modifiers.hardened
                                  ? Math.min(data.health.injuries, 5)
                                  : data.health.injuries) /
                                  10)
                  )
                : data.health.total

        data.health.percent = Math.round(
            (data.health.value / data.health.max) * 100
        )
        data.health.tick = Math.floor(data.health.total / 10)

        data.evasion = CalculateEvasions(
            data,
            actorData.flags?.ptu,
            actorData.items
        )
        data.capabilities = CalculateTrainerCapabilities(
            data.skills,
            actorData.items,
            data.stats.spe.stage.value + data.stats.spe.stage.mod,
            actorData.flags?.ptu
        )

        data.feats = {
            total: actorData.items.filter(
                (x) => x.type == 'feat' && !x.system.free
            ).length,
            max: 4 + Math.ceil(data.level.current / 2),
        }
        ...
        data.edges = {
            total: actorData.items.filter(
                (x) => x.type == 'edge' && !x.system.free
            ).length,
            max:
                4 +
                Math.floor(data.level.current / 2) +
                (data.level.current >= 2 ? 1 : 0) +
                (data.level.current >= 6 ? 1 : 0) +
                (data.level.current >= 12 ? 1 : 0),
        }

        data.ap.max = 5 + Math.floor(data.level.current / 5)

        data.initiative = {
            value: data.stats.spe.total + data.modifiers.initiative.total,
        }
        if (actorData.flags?.ptu?.is_paralyzed) {
            if (game.settings.get('ptu', 'errata'))
                data.initiative.value = Math.floor(data.initiative.value * 0.5)
        }
        if (data.modifiers.flinch_count?.value > 0) {
            data.initiative.value -= data.modifiers.flinch_count.value * 5
        }
    ...
    _preparePokemonData() {
        ...
        data.levelUpPoints =
            data.level.current + data.modifiers.statPoints.total + 10

        data.level.expTillNextLevel =
            data.level.current < 100
                ? game.ptu.data.levelProgression[data.level.current + 1]
                : game.ptu.data.levelProgression[100]
        data.level.percent = Math.round(
            ((data.level.exp -
                game.ptu.data.levelProgression[data.level.current]) /
                (data.level.expTillNextLevel -
                    game.ptu.data.levelProgression[data.level.current])) *
                100
        )
        ...
        data.stats = CalcBaseStats(
            data.stats,
            speciesData,
            data.nature.value,
            data.modifiers.baseStats
        )
        ...
        if (data.modifiers?.typeOverwrite) {
            ...
            for (const type of splitTypes) {
                if (
                    game.ptu.data.TypeEffectiveness[
                        Handlebars.helpers.capitalizeFirst(type.toLowerCase())
                    ]
                )
                    types.push(type)
            }
        }
        if (types.length == 0 && speciesData?.Type)
            types.push(...speciesData.Type)
        data.typing = types.length > 0 ? types : speciesData?.Type
        ...
        data.tp.max =
            (data.level.current > 0 ? Math.floor(data.level.current / 5) : 0) +
            1
        data.tp.pep.value = actorData.items.filter(
            (x) =>
                x.type == 'enhancement' &&
                x.system.origin?.toLowerCase() != 'pusher'
        ).length
        data.tp.pep.max =
            data.level.current > 0 ? Math.floor(data.level.current / 10) + 1 : 1
        ...
        data.capabilities = CalculatePokemonCapabilities(
            speciesData,
            actorData.items.values(),
            data.stats.spe.stage.value + data.stats.spe.stage.mod,
            Number(data.modifiers.capabilities?.total ?? 0),
            actorData.flags?.ptu
        )

        if (speciesData)
            data.egggroup =
                speciesData['Breeding Information']['Egg Group'].join(' & ')

        //TODO: Add skill background
        data.skills = CalculateSkills(
            data.skills,
            speciesData,
            actorData.items.filter((x) => x.type == 'enhancement'),
            data.background,
            data.modifiers.skillBonus.total
        )
        ...
        for (const item of actorData.items) {
            if (item.system.automation?.length > 0) {
                for (
                    let index = 0;
                    index < item.system.automation.length;
                    index++
                ) {
                    if (!item.system.automation[index].passive) continue
                    for (const target of item.system.automation[index]
                        .targets) {
                        if (!passives[target]) passives[target] = []
                        passives[target].push({
                            index: index,
    ...
    async rollMove(move, options = { moveId, bonusDamage, targets, APBonus }) {
        ...
        if (moveData.useCount == undefined) {
            for (const item of this.items) {
                if (
                    item.type == 'dexentry' ||
                    item.system.useCount != undefined
                )
                    continue
        ...
        if (
            moveData.frequency.toLowerCase().includes('daily') ||
            moveData.frequency.toLowerCase().includes('scene')
        ) {
            await move.update({
                'system.useCount':
                    Number(duplicate(moveData.useCount ?? 0)) + 1,
            })
        }

        // Set round properties if applicable
        if (
            move.getFlag('ptu', 'lastRoundUsed') !=
            (game.combat?.current?.round ?? 0)
        )
            await move.setFlag(
                'ptu',
                'lastRoundUsed',
        ...
        if (
            game.combat?.id &&
            move.getFlag('ptu', 'lastEncounterUsed') != game.combat?.id
        )
            await move.setFlag('ptu', 'lastEncounterUsed', game.combat?.id)

        // Apply Type Strategist
        if (attack.abilityBonuses.typeStratagistApplies) {
            const dr =
                this.system.health.value < this.system.health.total / 3 ? 10 : 5
            const aeAffliction = {
                ...
                changes: [
                    {
                        key: 'flags.ptu.has_type_strategist',
                        value: true,
                        mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                        priority: 50,
                    },
                    {
                        key: 'modifiers.damageReduction.physical.value',
                        value: dr,
                        mode: CONST.ACTIVE_EFFECT_MODES.ADD,
                        priority: 10,
                    },
                    {
                        key: 'modifiers.damageReduction.special.value',
                        value: dr,
                        mode: CONST.ACTIVE_EFFECT_MODES.ADD,
                        priority: 10,
                    },
        ...
        setTimeout(
            ...
            game.settings.get('ptu', 'dramaticTiming') == true
                ? move_animation_delay_ms
                : 0
        )
        ...
        if (
            moveData.name.toLowerCase() == 'transform' &&
            game.settings.get('ptu', 'autoTransform')
        ) {
            transform()
        }
    }

    async _performFullAttack(
        moveData,
        token,
        { bonusDamage, targets, moveName, APBonus }
    ) {
        if (!moveData) return
        ...
        moveData.doubleStrike = {
            is:
                (moveData.range?.toLowerCase().includes('double strike') ??
                    false) ||
                (moveData.range?.toLowerCase().includes('doublestrike') ??
                    false),
            hit1: undefined,
            hit2: undefined,
        }
        ...
        bonusDamage +=
            moveData.category.toLowerCase() == 'physical'
                ? this.system.modifiers?.damageBonus?.physical?.total +
                  this.system.stats.atk.total
                : moveData.category.toLowerCase() == 'special'
                  ? this.system.modifiers?.damageBonus?.special?.total +
                    this.system.stats.spa.total
                  : 0
        ...
        const damageBonuses = await calculateTotalDamageBonus(
            moveData,
            bonusDamage,
            currentWeather,
            abilityBonuses,
        ...
        // Do AC Roll
        const acRoll = await game.ptu.utils.combat
            .calculateAcRoll(moveData, this, APBonus)
            .evaluate({ async: true })
        if (moveData.doubleStrike.is === true) {
            const acRoll2 = await game.ptu.utils.combat
                .calculateAcRoll(moveData, this, APBonus)
                .evaluate({ async: true })
        ...
        let critType
        if (moveData.doubleStrike.is === true) {
            moveData.doubleStrike.hit1.crit = calculateCrit(
                moveData.doubleStrike.hit1.roll,
                this
            )
            moveData.doubleStrike.hit2.crit = calculateCrit(
                moveData.doubleStrike.hit2.roll,
                this
            )
            critType =
                moveData.doubleStrike.hit1.crit == CritOptions.CRIT_HIT &&
                moveData.doubleStrike.hit2.crit == CritOptions.CRIT_HIT
                    ? CritOptions.DOUBLE_CRIT_HIT
                    : moveData.doubleStrike.hit1.crit == CritOptions.CRIT_HIT ||
                        moveData.doubleStrike.hit2.crit == CritOptions.CRIT_HIT
                      ? CritOptions.CRIT_HIT
                      : moveData.doubleStrike.hit1.crit ==
                              CritOptions.CRIT_MISS &&
                          moveData.doubleStrike.hit2.crit ==
                              CritOptions.CRIT_MISS
                        ? CritOptions.CRIT_MISS
                        : CritOptions.NORMAL
        } else {
        ...
        const alwaysHits =
            critType == CritOptions.CRIT_HIT ||
            critType == CritOptions.DOUBLE_CRIT_HIT ||
            moveData.ac == '' ||
            moveData.ac == '--'
        if (critType == CritOptions.CRIT_MISS && alwaysHits)
            critType = CritOptions.NORMAL

        for (const attackInfo of Object.values(attacksData)) {
            if (moveData.doubleStrike.is === true) {
                moveData.doubleStrike.hit1.hit =
                    moveData.doubleStrike.hit1.roll.total >=
                    attackInfo.target.evasion.value
                moveData.doubleStrike.hit2.hit =
                    moveData.doubleStrike.hit2.roll.total >=
                    attackInfo.target.evasion.value
                attackInfo.isHit = alwaysHits
                    ? true
                    : critType == CritOptions.CRIT_MISS
                      ? false
                      : moveData.doubleStrike.hit1.hit ||
                        moveData.doubleStrike.hit2.hit
            } else {
                attackInfo.isHit = alwaysHits
                    ...
                    : critType == CritOptions.CRIT_MISS
                      ? false
                      : acRoll.total >= attackInfo.target.evasion.value
            }
        ...
        if (
            moveData.category.toLowerCase() != 'status' &&
            critType != CritOptions.CRIT_MISS
        ) {
            if (moveData.damageBase != '--' && moveData.damageBase) {
                damageRolls.normal = await this._calculateDamageRoll(
                    moveData,
                    CritOptions.NORMAL,
                    damageBonuses.total,
                    abilityBonuses,
                    this.system,
                    targets[0]?.actor?.system
                ).evaluate({ async: true })
                damageRolls.crit = await this._calculateDamageRoll(
                    moveData,
                    CritOptions.CRIT_HIT,
                    damageBonuses.total,
                    abilityBonuses,
        ...
        await game.ptu.utils.combat.takeAction(this, {
            actionType: match(moveData.range?.toLowerCase(), [
                {
                    test: (v) => v.includes('swift'),
                    result: (v) => ActionTypes.SWIFT,
                },
                {
                    test: (v) => v.includes('shift'),
                    result: (v) => ActionTypes.SHIFT,
                },
                {
                    test: (v) => v.includes('full action'),
                    result: (v) => ActionTypes.FULL,
                },
        ...
        function calculateTargetDifferences(user, moveData, targets) {
            ...
            for (const target of targets) {
                ...
                const rangeToTarget = canvas.grid.measureDistance(
                    userToken,
                    target,
                    { gridSpaces: true }
                )
                if (moveRange) {
                    function isInRange() {
                        switch (true) {
                            case moveRange.includes('pass'):
                                return 4
                            case moveRange.includes('melee'):
                                return Math.max(token.width, token.height)
                            case moveRange.includes('self'):
                                return Number.MAX_SAFE_INTEGER
                            case moveRange.includes('burst'):
                                return moveRange
                                    .slice(
                                        moveRange.indexOf('burst') +
                                            'burst'.length
                                    )
                                    .split(',')[0]
                                    .trim()
                            case moveRange.includes('cone'):
                                return moveRange
                                    .slice(
                                        moveRange.indexOf('cone') +
                                            'cone'.length
                                    )
                                    .split(',')[0]
                                    .trim()
                            case moveRange.includes('line'):
                                return moveRange
                                    .slice(
                                        moveRange.indexOf('line') +
                                            'line'.length
                                    )
                                    .split(',')[0]
                                    .trim()
                            case moveRange.includes('close blast'):
                                return moveRange
                                    .slice(
                                        moveRange.indexOf('close blast') +
                                            'close blast'.length
                                    )
                                    .split(',')[0]
                                    .trim()
                            case moveRange.includes('ranged blast'):
                                return (
                                    Number(moveRange.split(',')[0].trim()) +
                                    Number(
                                        moveRange
                                            .slice(
                                                moveRange.indexOf(
                ...
                switch (moveData.category.toLowerCase()) {
                    case 'physical':
                        attackData.target = {
                            ...
                            evasion: {
                                value: Math.max(
                                    target.actor.system.evasion.physical,
                                    target.actor.system.evasion.speed
                                ),
                                type: 'Physical/Speed',
                            },
                    ...
                    case 'special':
                        attackData.target = {
                            ...
                            evasion: {
                                value: Math.max(
                                    target.actor.system.evasion.special,
                                    target.actor.system.evasion.speed
                                ),
                                type: 'Special/Speed',
                            },
        ...
        function calculateAbilityBonuses(moveData, actor) {
            ...
            for (const item of actor.items.filter((x) => x.type == 'ability')) {
                switch (item.name.toLowerCase()) {
                    case 'technician':
                        output.hasTechnician = true
                        break
                    case 'adaptability':
                        output.hasAdaptability = true
                        break
                    case `last chance (${moveData.type.toLowerCase()})`:
                        output.lastChanceApplies = true
                        break
                    case `type strategist (${moveData.type.toLowerCase()})`:
                        output.typeStratagistApplies = true
                        break
                }
            }
            if (actor.system.heldItem?.toLowerCase()?.includes('booster')) {
                if (
                    actor.system.heldItem
                        .toLowerCase()
                        .includes(moveData.type.toLowerCase())
                ) {
        ...
        async function calculateTotalDamageBonus(
            moveData,
            bonusDamage,
            weather,
            abilityBonuses,
            ...
            if (moveData.damageBonus > 0) {
                ...
                modifierTexts.push(
                    `Including ${moveData.damageBonus >= 0 ? '+' : ''}${moveData.damageBonus} damage from [Move Damage Modifier Field]`
                )
            }
            ...
            if (game.keyboard.downKeys.has('ShiftLeft')) {
                await Dialog.confirm({
                    ...
                    yes: async (html) => {
                        const bonusTxt = html
                            .find('input[name="damage-bonus"]')
                            .val()

                        const bonus = !isNaN(Number(bonusTxt))
                            ? Number(bonusTxt)
                            : parseInt(
                                  (
                                      await new Roll(bonusTxt).roll({
                                          async: true,
                                      })
                                  ).total
                              )
                        if (!isNaN(bonus)) {
                            total += bonus
                            modifierTexts.push(
                                `Including ${bonus >= 0 ? '+' : ''}${bonus} damage from [Manual Modifier]`
                            )
                        }
                    },
                })
            ...
            if (abilityBonuses.typeBoosterApplies) {
                total += 5
                modifierTexts.push(
                    `Including +5 damage from ${moveData.type} Booster!`
                )
            }
            // Last Chance
            if (abilityBonuses.lastChanceApplies) {
                if (actor.system.health.value < actor.system.health.total / 3) {
                    total += 10
                    modifierTexts.push(
                        `Including +10 damage from Last Chance (${moveData.type}) while below 1/3rd HP!`
                    )
                } else {
                    total += 5
                    modifierTexts.push(
                        `Including +5 damage from Last Chance (${moveData.type})`
                    )
                }
            }
            // Weather
            if (weather != 'Clear') {
                switch (true) {
                    case moveData.type == 'Water' && weather == 'Rainy':
                        total += 5
                        modifierTexts.push(
                            `including +5 damage from Rainy weather!`
                        )
                        break
                    ...
                    case moveData.type == 'Water' && weather == 'Sunny':
                        total -= 5
                        modifierTexts.push(
                            `including -5 damage from Sunny weather!`
                        )
                        break
            ...
            if (abilityBonuses.typeStratagistApplies) {
                modifierTexts.push(
                    `Type Strategist (${moveData.type}) activated!`
                )
            }
        ...
        function calculateCrit(acRoll, actor) {
            ...
            return diceResult === 1
                ? CritOptions.CRIT_MISS
                : diceResult >= 20 - actor.system.modifiers.critRange?.total
                  ? CritOptions.CRIT_HIT
                  : CritOptions.NORMAL
        }

        function calculateFiveStrike() {
            return FiveStrikeHitsDictionary[
                Math.floor(Math.random() * (8 - 1 + 1)) + 1
            ]
        }
    }

    _calculateDamageRoll(
        moveData,
        critType,
        damageBonus,
        abilityBonuses,
        ...
        if (moveData.doubleStrike.is === true) {
            ...
            hitCount = 0
            if (!targetData || moveData.doubleStrike.hit1.hit === true)
                hitCount++
            if (!targetData || moveData.doubleStrike.hit2.hit === true)
                hitCount++
        }
        ...
        if (
            abilityBonuses.hasTechnician &&
            (moveData.doubleStrike.is === true ||
                moveData.fiveStrike.is === true ||
                db <= 6)
        ) {
            dbBonus += 2
        }
        ...
        if (
            this.system.typing &&
            (moveData.type == this.system.typing[0] ||
                moveData.type == this.system.typing[1])
        ) {
        ...
        if (moveData.name.toLowerCase().includes('stored power')) {
            const dbBonusFromStages = Math.min(
                20 - db,
                ((actorData.stats.atk.stage.value +
                    actorData.stats.atk.stage.mod <
                0
                    ? 0
                    : actorData.stats.atk.stage.value +
                      actorData.stats.atk.stage.mod) +
                    (actorData.stats.spa.stage.value +
                        actorData.stats.spa.stage.mod <
                    0
                        ? 0
                        : actorData.stats.spa.stage.value +
                          actorData.stats.spa.stage.mod) +
                    (actorData.stats.def.stage.value +
                        actorData.stats.def.stage.mod <
                    0
                        ? 0
                        : actorData.stats.def.stage.value +
                          actorData.stats.def.stage.mod) +
                    (actorData.stats.spd.stage.value +
                        actorData.stats.spd.stage.mod <
                    0
                        ? 0
                        : actorData.stats.spd.stage.value +
                          actorData.stats.spd.stage.mod) +
                    (actorData.stats.spe.stage.value +
                        actorData.stats.spe.stage.mod <
                    0
                        ? 0
                        : actorData.stats.spe.stage.value +
                          actorData.stats.spe.stage.mod)) *
        ...
        if (moveData.name.toLowerCase().includes('punishment')) {
            const dbBonusFromStages = Math.min(
                12 - db,
                (targetData?.stats?.atk.stage.value +
                    targetData?.stats?.atk.stage.mod <
                0
                    ? 0
                    : targetData?.stats?.atk.stage.value +
                      targetData?.stats?.atk.stage.mod) +
                    (targetData?.stats?.spa.stage.value +
                        targetData?.stats?.spa.stage.mod <
                    0
                        ? 0
                        : targetData?.stats?.spa.stage.value +
                          targetData?.stats?.spa.stage.mod) +
                    (targetData?.stats?.def.stage.value +
                        targetData?.stats?.def.stage.mod <
                    0
                        ? 0
                        : targetData?.stats?.def.stage.value +
                          targetData?.stats?.def.stage.mod) +
                    (targetData?.stats?.spd.stage.value +
                        targetData?.stats?.spd.stage.mod <
                    0
                        ? 0
                        : targetData?.stats?.spd.stage.value +
                          targetData?.stats?.spd.stage.mod) +
                    (targetData?.stats?.spe.stage.value +
                        targetData?.stats?.spe.stage.mod <
                    0
                        ? 0
                        : targetData?.stats?.spe.stage.value +
                          targetData?.stats?.spe.stage.mod)
    ...
    async _commandCheck(trainerActor) {
        ...
        const ALTERNATE_COMMAND_SKILL_FEATURES = game.settings.get(
            'ptu',
            'pokepsychologistCanReplaceCommand'
        )
            ? {
        ...
        const commandSkill = {
            name: 'command',
            rank: () =>
                trainerActor.system.skills[commandSkill.name].value.total,
            mod: () =>
                trainerActor.system.skills[commandSkill.name].modifier.total,
        }
        ...
        for (const item of trainerActor.items) {
            ...
            if (
                trainerActor.system.skills[
                    ALTERNATE_COMMAND_SKILL_FEATURES[item.name]
                ].value.total +
                    trainerActor.system.skills[
                        ALTERNATE_COMMAND_SKILL_FEATURES[item.name]
                    ].modifier.total *
                        0.1 >
                commandSkill.rank() + commandSkill.mod() * 0.1
            )
...
export function GetSpeciesData(species) {
    ...
    if (species) {
        ...
        if (parseInt(species)) {
            ...
            if (!preJson) {
                preJson = game.ptu.data.customSpeciesData.find(
                    (x) => x.number == species
                )
            ...
            if (species.toLowerCase().includes('oricorio-')) {
                ...
                //I FIXED YOUR FUCKING CALLS YOU BRAINDEAD FUCKS, _id NEEDS A FUCKING ?
            }
            if (!preJson) {
                preJson = game.ptu.data.pokemonData.find(
                    (x) => x._id?.toLowerCase() === species.toLowerCase()
                )
            }
            if (!preJson) {
                preJson = game.ptu.data.customSpeciesData.find(
            ...
            if (!preJson) {
                preJson = game.ptu.data.pokemonData.find(
                    (x) =>
                        x._id?.toLowerCase() ===
                        species.toLowerCase() + '-normal'
                )
                if (!preJson) {
                    preJson = game.ptu.data.pokemonData.find(
                        (x) =>
                            x._id?.toLowerCase() ===
                            species.toLowerCase() + '-male'
                    )
                    if (!preJson) return null
                }
            }
