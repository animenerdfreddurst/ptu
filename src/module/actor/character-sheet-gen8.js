...
export class PTUGen8CharacterSheet extends ActorSheet {
    ...
    getData() {
        ...
        data['totalWealth'] = this.actor.itemTypes.item.reduce(
            (total, item) =>
                total +
                (!isNaN(item.system.cost)
                    ? item.system.cost * (item.system.quantity ?? 0)
                    : 0),
            this.actor.system.money
        )
    ...
    _getHeaderButtons() {
        ...
        if (game.user.isGM) {
            buttons.unshift({
                ...
                icon: 'fas fa-edit',
                onclick: () =>
                    new game.ptu.config.Ui.CharacterNotesForm.documentClass(
                        this.actor,
                        { submitOnClose: true, closeOnSubmit: false }
                    ).render(true),
            })
        }
    ...
    _prepareCharacterItems(sheetData) {
        ...
        for (const category of Object.keys(items_categorized)) {
            if (actor.system.item_categories[category] === undefined)
                actor.system.item_categories[category] = true
        }
    ...
    activateListeners(html) {
        ...
        html.find('.effect-delete').click((ev) => {
            ...
            $(ev.currentTarget)
                .parents('.swsh-box')
                .slideUp(200, () => this.render(false))
            setTimeout(() => effect.delete(), 150)
        })

        html.find('input[data-item-id]').on('change', (e) =>
            this._updateItemField(e)
        )
        ...
        document
            .getElementsByName('system.ap.value input')[0]
    ...
    async _onDropItem(event, data) {
        ...
        if (oldItem && oldItem.id != itemData.id && oldItem.system.quantity) {
			// have to use parseInt because the people who made this have shit for brains and are storing the quantity as text... Kill me
            const quantity = parseInt(duplicate(oldItem.system.quantity))
    ...
    async _onDexSort(event) {
        ...
        const sortId = (a, b) =>
            parseInt(game.ptu.utils.species.get(a.name).ptuNumber) -
            parseInt(game.ptu.utils.species.get(b.name).ptuNumber)

        const entries = this.actor.itemTypes.dexentry.sort(
            dataset.sort === 'name' ? sortName : sortId
        )
        const itemsToUpdate = []
    ...
    async _onRoll(event) {
        ...
        if (dataset.roll) {
            ...
            if (event.altKey && this.useAP()) {
                // Only if AP are available
            ...
            if (event.shiftKey) {
                const extra = await new Promise((resolve, reject) => {
                    Dialog.confirm({
                        ...
                        yes: async (html) => {
                            const bonusTxt = html
                                .find('input[name="skill-roll-modifier"]')
                                .val()
    ...
    async _onSaveRoll(event = new Event('void')) {
        ...
        messageData.content = await renderTemplate(
            `/systems/ptu/templates/chat/save-check.hbs`,
            messageData
        )
    ...
    _onMoveRoll(event, { actor, item } = {}) {
        ...
        move.data = PrepareMoveData(
            actor ? actor.system : this.actor.system,
            move.data
        )
    ...
    useAP(value = 1) {
        ...
        ui.notifications.error(
            `${this.actor.name} does not have enough AP for this action.`
        )
        return false
    }
