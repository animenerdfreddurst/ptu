...
import {
  PTUGen8CharacterSheet,
  CalculateAcRoll,
} from "./module/actor/character-sheet-gen8.js";
import { PTUGen8PokemonSheet } from "./module/actor/pokemon-sheet-gen8.js";
import { PTUItem } from "./module/item/item.js";
import { PTUItemSheet } from "./module/item/item-sheet.js";
import { PTUEdgeSheet } from "./module/item/edge-sheet.js";
import { PTUFeatSheet } from "./module/item/feat-sheet.js";
import { PTUMoveSheet } from "./module/item/move-sheet.js";
import { measureDistances } from "./module/canvas.js";
import { levelProgression } from "./module/data/level-progression.js";
import { pokemonData } from "./module/data/species-data.js";
import { natureData } from "./module/data/nature-data.js";
import {
  insurgenceData,
  sageData,
  uraniumData,
} from "./module/data/fangame-species-data.js";
import { DbData } from "./module/data/db-data.js";
import { TypeEffectiveness } from "./module/data/effectiveness-data.js";
import { PTUPokemonCharactermancer } from "./module/forms/charactermancer-pokemon-form.js";
import { PTUDexDragOptions } from "./module/forms/dex-drag-options-form.js";
// import { PTUCustomSpeciesEditor } from './forms/custom-species-editor-form.js'
import { PTUCustomSpeciesEditor } from "./module/forms/cse-form.js";
import { PTUCustomTypingEditor } from "./module/forms/cte-form.js";
import { PTUCharacterNotesForm } from "./module/forms/character-notes-form.js";
import { RollWithDb } from "./module/utils/roll-calculator.js";
import {
  PrepareCustomSpecies,
  UpdateCustomSpeciesData,
} from "./module/custom-species.js";
import {
  InitCustomTypings as initCustomTypings,
  UpdateCustomTypings,
} from "./module/custom-typings.js";
import { ChangeLog } from "./module/forms/changelog-form.js";
import {
  ...
  TakeAction,
} from "./module/combat/damage-calc-tools.js";
import {
  CreateMonParser,
  GetSpeciesArt,
} from "./module/utils/species-command-parser.js";
import { FinishDexDragPokemonCreation } from "./module/utils/species-command-parser.js";
import { GetRandomNature } from "./module/utils/random-nature-generator.js";
import { GiveRandomAbilities } from "./module/utils/random-abilities-generator.js";
import { GiveLatestMoves } from "./module/utils/latest-moves-generator.js";
import { ApplyEvolution } from "./module/utils/calculate-evolution.js";
import { GiveCapabilities } from "./module/utils/capability-generator.js";
import {
  ...
  BaseStatsWithNature,
  ApplyLevelUpPoints,
} from "./module/utils/calculate-stat-distribution.js";
import { GetOrCreateCachedItem } from "./module/utils/cache-helper.js";
import { ActorGenerator } from "./module/utils/actor-generator.js";
import { Afflictions } from "./module/combat/effects/afflictions.js";
import PTUCombat from "./module/combat/combat.js";
import {
  PTUCombatOverrides,
  PTUCombatTrackerOverrides,
} from "./module/combat/ptu_overrides.js";
import Api from "./module/api/api.js";
import RenderDex, { AddMontoPokedex } from "./module/utils/pokedex.js";
import TMsData from "./module/data/tm-data.js";
import PTUActiveEffectConfig from "./module/forms/active-effect-config.js";
import getTrainingChanges from "./module/data/training-data.js";
import { PTUSettings, PTUSettingCategories } from "./module/forms/settings.js";
import {
  LoadSystemSettings as loadSystemSettings,
  SetAccessabilityFont,
} from "./module/settings.js";
import Store from "./module/api/front-end/lib/store.js";
import Component from "./module/api/front-end/lib/component.js";
import { PTUSidebar } from "./module/sidebar/sidebar-form.js";
import "./module/utils/item-piles-compatibility-handler.js";
import "./module/utils/drag-ruler-compatibility-handler.js";
import { ThrowPokeball } from "./module/combat/effects/pokeball_effects.js";
import { LANG } from "./module/utils/language-helper.js";
import logging from "./module/helpers/logging.js";
import {
  registerHandlebars,
  preloadHandlebarsTemplates,
} from "./module/helpers/handlebars.js";
import { PTRSearch } from "./module/ptr-search/ptr-search.js";
...
function registerSheets() {
  ...
  Actors.registerSheet("ptu", ptu.config.Actor.sheetClasses.character, {
    types: ["character"],
    makeDefault: true,
  });
  Actors.registerSheet("ptu", ptu.config.Actor.sheetClasses.pokemon, {
    types: ["pokemon"],
    makeDefault: true,
  });
  ...
  Items.registerSheet("ptu", ptu.config.Item.sheetClasses.move, {
    types: ["move"],
    makeDefault: true,
  });
  Items.registerSheet("ptu", ptu.config.Item.sheetClasses.edge, {
    types: ["edge"],
    makeDefault: true,
  });
  Items.registerSheet("ptu", ptu.config.Item.sheetClasses.feat, {
    types: ["feat"],
    makeDefault: true,
  });
...
export function PrepareMoveData(actorData, move) {
  ...
  move.stab =
    move.owner?.type &&
    move.owner.type.filter((t) => t == move.type).length > 0;
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (["Delete", "Backspace"].includes(key)) {
      ...
      if (canvas.ready && canvas.activeLayer instanceof PlaceablesLayer) {
        ...
        const objects = layer.options.controllableObjects
          ? layer.controlled
          : layer._hover
          ? [layer._hover]
          : [];
        ...
        const uuids = objects.reduce((uuids, o) => {
          if (o.locked || o.document.canUserModify(game.user, "delete"))
            return uuids;
          if (!o.document.actor.canUserModify(game.user, "delete"))
            return uuids;
          uuids.push(o.document?.uuid ?? o.uuid);
          return uuids;
        }, []);
        if (uuids.length) {
          if (Hooks.call("prePlayerDeleteToken", uuids))
            return game.ptu.utils.api.gm.tokensDelete(uuids);
        }
      }
    }
  });
...
Hooks.once("ready", async function () {
  ...
  game.ptu.data.items = Array.from(
    new Set(
      game.items
        .filter((x) => x.type == "item")
        .concat(await game.packs.get("ptu.items").getDocuments())
    )
  );
  ...
  game.socket.on("system.ptu", async (data) => {
    if (data == null) return;
    if (
      data == "RefreshCustomSpecies" ||
      (data == "ReloadGMSpecies" && game.user.isGM)
    )
      Hooks.callAll("updatedCustomSpecies");
    if (data == "RefreshCustomTypings") Hooks.callAll("updatedCustomTypings");
    if (data == "RefreshCustomTypingsAndActors")
      Hooks.callAll("updatedCustomTypings", { updateActors: true });
    if (data.type) {
  ...
  /** Display Changelog */
  if (
    game.settings.get("ptu", "dismissedVersion")[game.userId] !== LATEST_VERSION
  ) {
...
Hooks.on("dropActorSheetData", function (actor, sheet, itemDropData) {
  ...
  const updateActorBasedOnSpeciesItem = function (item) {
    if (item.name) {
      log(
        `Updating Species based on Dex Drop (${actor.system.species} -> ${item.name})`
      );
      actor
        .update({ "data.species": item.name })
        .then((x) => log("Finished Updating Species based on Dex Drop"));
    } else if (item.system.id) {
      log(
        `Updating Species based on Dex Drop (${actor.system.species} -> ${item.system.id})`
      );
      actor
        .update({ "data.species": item.system.id })
        .then((x) => log("Finished Updating Species based on Dex Drop"));
    }
  };

  if (
    itemDropData.uuid.includes("dex-entries") ||
    itemDropData.uuid.includes("uranium-and-sage-dex")
  ) {
...
Hooks.on("renderSettings", (app, html) => {
  html
    .find("#settings-game")
    .after($(`<h2>PTU System Settings</h2><div id="ptu-options"></div>`));
  ...
  if (game.user.isGM) {
    ...
    html
      .find('button[data-action="ptu-custom-species-editor"')
      .on("click", (_) =>
        new game.ptu.config.Ui.CustomSpeciesEditor.documentClass().render(true)
      );
    $("#ptu-options").append(
      $(
    ...
    html
      .find('button[data-action="ptu-custom-typing-editor"')
      .on("click", (_) =>
        new game.ptu.config.Ui.CustomTypingEditor.documentClass().render(true)
      );
  }
});
...
async function createPTUMacro(data, slot) {
  ...
  let macro = game.macros.contents.find(
    (m) => m.name === `${actor.name}'s ${item.name}` && m.command === command
  );
  if (!macro) {
    macro = await Macro.create({
...
function rollItemMacro(actorId, itemId, sceneId, tokenId) {
  const isTokenActor =
    sceneId && sceneId != "null" && tokenId && tokenId != "null";
  ...
  if (isTokenActor) {
    const token = game.scenes
      .get(sceneId)
      ?.data?.tokens?.find((x) => x?.id == tokenId);
    if (!token)
      return ui.notifications.warn(
        `Scene or token no longer exists. Macro is invalid.`
      );
    actorData = mergeObject(actorData, token.actorData);
  }

  if (!actor)
    return ui.notifications.warn(`Couldn't find actor with ID ${actorId}`);

  const item =
    isTokenActor && actorData.items
      ? actorData.items.find((x) => x.id == itemId)
      : actor.items.get(itemId);
  if (!item)
    return ui.notifications.warn(
      `Actor ${actor.name} doesn't have an item with ID ${itemId}`
    );
  ...
  switch (item.type) {
    ...
    default:
      return ui.notifications.warn(
        `I'm sorry, macro support has yet to be added for '${item.type}s'`
      );
  }
}
...
async function _onPokedexMacro() {
  ...
  for (let token of game.user.targets.size > 0
    ...
    if (addToDex && !game.user.isGM) {
      if (!game.user.character)
        return ui.notifications.warn(
          "Please make sure you have a trainer as your Selected Player Character"
        );
    ...
    switch (permSetting) {
      ...
      case 2: {
        //pokemon description only
        game.ptu.utils.dex.render(token.actor.system.species);
        break;
      }
      case 3: {
        // Only owned tokens
        game.ptu.utils.dex.render(
          token.actor.system.species,
          token.owner ? "full" : "desc"
        );
        break;
      }
      case 4: {
        ...
        game.ptu.utils.dex.render(
          token.actor.system.species,
          game.user.character.itemTypes.dexentry.some(
            (entry) =>
              entry.system.owned &&
              entry.data.name.toLowerCase() === monData?._id?.toLowerCase()
          )
            ? "full"
            : "desc"
        );
      ...
      case 5: {
        // GM Prompt
        const result = await game.ptu.utils.api.gm.dexScanRequest(
        ...
        switch (result) {
          case "false": {
            return ui.notifications.info(
              game.i18n.localize("PTU.DexScan.Denied")
            );
          }
          case "timeout": {
            return ui.notifications.warn(
              game.i18n.localize("PTU.DexScan.Timeout")
            );
          }
          case "description": {
            game.ptu.utils.dex.render(token.actor.system.species);
...
Hooks.on("updateInitiative", function (actor) {
  ...
  if (init + decimal != combatant.initiative) {
    game.combats.active.setInitiative(
      combatant.id,
      init >= 0 ? init + decimal : (Math.abs(init) + decimal) * -1
    );
  }
  return true;
});
...
Hooks.on("preCreateItem", (item, itemData, options, sender) => {
  ...
  const entry = item.parent.itemTypes.dexentry.find(
    (e) => e.system.id == item.system.id
  );
  if (entry) {
    log(
      "Dex entry already exists, skipping. This may throw an error, which can be ignored."
    );
    return false;
  }
});
...
Hooks.on("preCreateItem", async function (item, data, options, sender) {
  ...
  const TMs = speciesData["TM Move List"].map((x) =>
    Handlebars.helpers.tmName(x)
  );
  const TutorMoves = speciesData["Tutor Move List"].map((x) =>
    x.replace("(N)", "")
  );
...
Hooks.on(
  "renderTokenConfig",
  (config, html, options) =>
    (html.find("[name='actorLink']").siblings()[0].outerHTML =
      "<label>Link Actor Data <span class='readable p10'>Unlinked actors are not supported by the system</span></label>")
);
...
Hooks.on("renderTokenHUD", (app, html, data) => {
  ...
  if (game.modules.get("ptu-movement-info")?.active) {
    ui.notification.warn(
      "Thanks for using the PTU Movement info module! This module is now included in the PTR system and will no longer by updated. Please ask your GM to disable to PTY Movement Info module."
    );
    return;
  }
  ...
  if (game.modules.get("barbrawl")?.active) {
    ...
    ui.notifications.warn(
      "Movement icons are not compatible with the barbrawl module. Please disable the barbrawl module to use movement icons.\nYou can disable movement icons in the PTU settings > Player Preferences to avoid seeing this message."
    );
    return;
  }
  ...
  const buttons = [];
  for (const [c, i] of Object.entries(capabilitiesMap)) {
    //c=capability, i=icon
    ...
    buttons.push(
      `<div class="control-icon chalk-icon" title="${c}: ${val}"><i class="${i}"></i>${val}</div>`
    );
  }

  html.find(".col.middle").before(
    // if the actor uses a 2nd bar increase height.
    `<div class="col middle" style="top: -${
      html.find(".bar2").html().trim() ? 105 : 90
    }px;">
...
Hooks.on("preUpdateActor", async (oldActor, changes, options, sender) => {
  ...
  changes.system.level.milestones = changeValue(
    changes.system?.level?.milestones,
  ...
  changes.system.level.miscExp = changeValue(
    ...
    oldActor.system.level.miscExp
  );

  //hp
  changes.system.health.value = changeValue(
    changes.system?.health?.value,
    oldActor.system.health.value
  );

  //tempHp
  changes.system.tempHp.value = changeValue(
    changes.system?.tempHp?.value,
    oldActor.system.tempHp.value
  );

  //tempHpMax
  changes.system.tempHp.max = changeValue(
    changes.system?.tempHp?.max,
    oldActor.system.tempHp.max
  );
...
Hooks.on("renderChatMessage", (message, html, data) => {
  setTimeout(() => {
    $(html)
      .find(".dex-desc-button")
      .on("click", (event) => showPlayerDexEntry(event));
  }, 500);
});
...
export async function showPlayerDexEntry(event) {
  ...
  const userId = game.users.find(
    (u) => (u.character = game.actors.getName(trainername) && !u.isGM)
  )._id;
...
async function updateItems(actors = []) {
  ...
  for (const actor of actors) {
    ...
    for (const item of actor.items) {
      console.log(
        `Checking ${item.name} (${item.uuid}) with system version ${item._stats?.systemVersion}`
      );
      // Check if the item's system version is older than the current version
      if (
        isNewerVersion(game.system.version, item._stats?.systemVersion ?? 0)
      ) {
        ...
        const index = compendium.index.getName(
        ...
        if (
          newItem &&
          isNewerVersion(
            newItem._stats?.systemVersion ?? 0,
            item._stats?.systemVersion ?? 0
          )
        ) {
          console.log(
            `Updating ${item.name} (${item.uuid}) from ${
              item._stats?.systemVersion ?? 0
            } to ${newItem._stats?.systemVersion ?? 0}`
          );
